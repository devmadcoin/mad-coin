...
function LiveTicker() {
  ...
  const buyRatio = stats.buys !== "—" && stats.sells !== "—"
    ? Math.round((parseInt(stats.buys) / (parseInt(stats.buys) + parseInt(stats.sells))) * 100)
    : 50;

  return (
    <div className="fixed top-0 left-0 right-0 z-[60] bg-black/95 border-b border-red-500/20">
      <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between gap-4 overflow-x-auto">
        <div className="flex items-center gap-6 min-w-max">
          <span className="text-red-500 text-[10px] font-black uppercase tracking-widest shrink-0">$MAD LIVE</span>
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="text-white/40 text-[10px] font-bold uppercase">PRICE</span>
            <span className="text-sm font-black text-white">{stats.price}</span>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="text-white/40 text-[10px] font-bold uppercase">24H</span>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="text-white/40 text-[10px] font-bold uppercase">BUYS</span>
            <span className="text-sm font-black text-green-400">{stats.buys}</span>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="text-white/40 text-[10px] font-bold uppercase">SELLS</span>
            <span className="text-sm font-black text-red-400">{stats.sells}</span>
          </div>
          <div className="hidden sm:flex items-center gap-2 shrink-0">
            <div className="w-24 h-2 rounded-full bg-white/10 overflow-hidden">
              <div className="h-full rounded-full bg-gradient-to-r from-green-500 to-red-500" style={{ width: "100%" }} />
              <div className="relative -top-2 h-full bg-green-500 rounded-full" style={{ width: `${buyRatio}%` }} />
            </div>
            <span className="text-[10px] font-bold text-green-400">{buyRatio}% BUY</span>
          </div>
        </div>
...
function GiantButton({ href, children, primary = false, onClick }: { href?: string; children: React.ReactNode; primary?: boolean; onClick?: () => void }) {
  const className = [
    ...
    primary
      ? "bg-red-500 text-white hover:bg-red-400 hover:scale-[1.03] shadow-[0_0_60px_rgba(255,0,0,0.35)]"
      : "border-2 border-white/20 bg-white/[0.04] text-white hover:border-white/40 hover:bg-white/[0.08] hover:scale-[1.03]",
  ].join(" ");
...
function StepCard({ number, title, icon, link, linkText }: { number: string; title: string; icon: React.ReactNode; link: string; linkText: string }) {
  return (
    <div className="flex flex-col items-center text-center p-6">
      ...
      <p className="text-red-500 font-black text-2xl mb-2">{number}</p>
      <p className="text-white font-black text-lg mb-3">{title}</p>
      <a href={link} target="_blank" rel="noreferrer" className="text-sm font-bold text-red-400 hover:text-red-300 transition-colors inline-flex items-center gap-1">
        {linkText}
  ...
  return (
    <div className="relative">
      <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
        ...
        <Link href={LINKS.memes} className="snap-start shrink-0 w-[280px] sm:w-[320px] rounded-[24px] border border-red-500/20 bg-red-500/5 flex items-center justify-center">
          <div className="text-center p-8">
            <p className="text-4xl mb-2">→</p>
            <p className="text-red-400 font-black text-lg">VIEW ALL</p>
            <p className="text-white/40 text-xs mt-1">29 MEMES</p>
          </div>
        </Link>
      </div>
      <div className="text-center mt-2">
        <p className="text-white/30 text-xs font-bold tracking-widest">← SWIPE TO EXPLORE →</p>
      </div>
    </div>
  );
}
...
function TeaserCard({ href, title, sub, emoji }: { href: string; title: string; sub: string; emoji: string }) {
  return (
    <Link href={href} className="group flex flex-col items-center text-center p-8 rounded-[28px] border border-white/10 bg-white/[0.02] hover:border-red-500/30 hover:bg-red-500/5 transition-all duration-300">
      ...
      <p className="mt-4 text-red-400 font-bold text-sm group-hover:translate-x-1 transition-transform">Enter →</p>
    </Link>
  );
}
...
function ConfessionBanner() {
  ...
  return (
    <div className="rounded-[24px] border border-white/10 bg-white/[0.02] p-6 sm:p-8 text-center">
      <p className="text-white/40 text-[10px] font-bold uppercase tracking-[0.3em] mb-4">Anonymous Confession</p>
      <p className="text-xl sm:text-2xl font-black text-white leading-tight">"{confessions[index]}"</p>
      <p className="mt-4 text-white/30 text-xs font-bold">— Holder #{273 + index}</p>
    </div>
  );
}

/* ═══════════════════════════════════════════════════════════
 REDESIGNED HOMEPAGE — Drunk-person navigable
 ═══════════════════════════════════════════════════════════ */
    ...
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <main className="pt-16">
        ...
        <section className="min-h-[100dvh] flex flex-col items-center justify-center px-4 py-20 relative">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,0,0,0.12),transparent_50%)]" />
          
          <div className="relative z-10 text-center max-w-4xl mx-auto">
            <p className="text-red-500 text-[10px] font-black uppercase tracking-[0.4em] mb-6">Solana • 513M Supply • Limited</p>
            
            <h1 className="text-[18vw] sm:text-[14vw] md:text-[12vw] font-black leading-[0.8] tracking-[-0.04em] text-white">
              $MAD
            </h1>
            
            <p className="mt-6 text-xl sm:text-2xl font-bold text-white/70 max-w-lg mx-auto leading-relaxed">
          ...
          <div className="absolute bottom-8 left-0 right-0 text-center">
            <p className="text-white/20 text-xs font-bold tracking-widest">SCROLL DOWN ↓</p>
          </div>
        </section>

        {/* ═══════════════════════════════════════════
         THE NUMBERS — Proof it's alive
        ═══════════════════════════════════════════ */}
        <section className="py-20 px-4 sm:px-6">
          <div className="max-w-5xl mx-auto">
            <div className="text-center mb-12">
              <p className="text-red-500 text-[10px] font-black uppercase tracking-[0.4em] mb-3">24 Hour Activity</p>
            ...
            {liveStats && liveStats.buys !== "—" && (
              <div className="mt-6 rounded-[24px] border border-white/10 bg-white/[0.02] p-6">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-white/40 text-xs font-bold uppercase">Buy Pressure</span>
        ...
        {/* ═══════════════════════════════════════════
         HOW TO GET IT — 3 steps, zero confusion
        ═══════════════════════════════════════════ */}
        <section className="py-20 px-4 sm:px-6 bg-[linear-gradient(180deg,transparent,rgba(255,0,0,0.03),transparent)]">
          <div className="max-w-5xl mx-auto">
            <div className="text-center mb-12">
              <p className="text-red-500 text-[10px] font-black uppercase tracking-[0.4em] mb-3">Get Started</p>
            ...
            <div className="mt-8 rounded-[24px] border-2 border-red-500/20 bg-red-500/[0.03] p-6 sm:p-8">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="text-center sm:text-left">
                  <p className="text-white/40 text-xs font-bold uppercase tracking-widest mb-2">Contract Address</p>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={[
                      ...
                      copied ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30",
                    ].join(" ")}
                  >
        ...
        {/* ═══════════════════════════════════════════
         THE ART — Swipeable dopamine
        ═══════════════════════════════════════════ */}
        <section className="py-20 px-4 sm:px-6">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-10">
              <p className="text-red-500 text-[10px] font-black uppercase tracking-[0.4em] mb-3">$MAD Art</p>
              <h2 className="text-4xl sm:text-5xl font-black text-white">The Gallery</h2>
              <p className="mt-3 text-white/50 text-lg">Swipe. Laugh. Share.</p>
            </div>
            <MemeScroll />
          </div>
        </section>
        ...
        {/* ═══════════════════════════════════════════
         EXPLORE — Teasers for other tabs
        ═══════════════════════════════════════════ */}
        <section className="py-20 px-4 sm:px-6 bg-[linear-gradient(180deg,transparent,rgba(255,0,0,0.02),transparent)]">
          <div className="max-w-5xl mx-auto">
            <div className="text-center mb-12">
              <p className="text-red-500 text-[10px] font-black uppercase tracking-[0.4em] mb-3">Explore</p>
        ...
        {/* ═══════════════════════════════════════════
         FINAL CTA — Last chance to buy
        ═══════════════════════════════════════════ */}
        <section className="py-24 px-4 sm:px-6">
          <div className="max-w-3xl mx-auto text-center">
            <h2 className="text-5xl sm:text-6xl font-black text-white mb-6">Still Here?</h2>
            <p className="text-xl text-white/60 mb-10">The chart doesn't care about your hesitation.</p>
            <GiantButton href={LINKS.buy} primary>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
              GET $MAD NOW
            </GiantButton>
            <div className="mt-8 flex items-center justify-center gap-6">
              <a href={LINKS.telegram} target="_blank" rel="noreferrer" className="text-white/40 hover:text-white transition-colors">
              ...
              <a href={LINKS.x} target="_blank" rel="noreferrer" className="text-white/40 hover:text-white transition-colors">
        ...
        <section className="px-4 sm:px-6 pb-20">
          <div className="max-w-3xl mx-auto rounded-[24px] border border-yellow-400/15 bg-yellow-500/[0.05] px-6 py-8 text-center">
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-yellow-300/70 mb-3">Risk Notice</p>
